...
function COVEApi (options) {
    ...
    this.base_url_https = options.base_url_https || 'https://api.pbs.org/cove/v1';
...
COVEApi.prototype.signUrl = function (cannonical_url, timestamp, nonce, method) {
    var sig = this.generateSignature(cannonical_url, timestamp, nonce, method),
        signed_url = cannonical_url,
        consumer_key = this.api_id;

    // Build the url + timestamp + consumer_key + nonce + sig
    signed_url += "&timestamp=" + timestamp;
    signed_url += "&consumer_key=" + consumer_key;
    signed_url += "&nonce=" + nonce;
    signed_url += '&signature=' + sig;
    return this.normalizeUrl(signed_url);
};
...
COVEApi.prototype.request = function (baseurl, options){
    ...
    if (options.auth_using_headers) {
        ...
        req_options.url = baseurl;
        logger.debug("Auth using headers");
    } else {
        // signature in the url
        req_options.url = this.signUrl(url, timestamp, nonce, method);
    }

    logger.debug('cove-api:url', req_options.url);
