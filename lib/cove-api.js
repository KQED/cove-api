...
COVEApi.prototype.request = function (baseurl, options){
    ...
    // build url
    var normalized_url = this.normalizeUrl(baseurl, timestamp, nonce);

    // build signature
    var signed_url = this.signUrl(normalized_url, timestamp, nonce, method);
    ...
    // make request
    req(signed_url, function (err, res, body) {
        if (err) {
            logger.error(err);
